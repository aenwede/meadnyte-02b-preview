(() => {
  ...
  const state={poem:0,cue:0,cues:[],original:null,duration:0,loop:false,dirty:false,peaks:[],history:[]};
  ...
  async function loadPoem(index){
    state.poem=(index+poems.length)%poems.length; state.cue=0; state.loop=false; state.history=[]; updateUndo(); $('[data-loop]').textContent='Loop cue: off'; select.value=state.poem;
  ...
  $('[data-play]').onclick=()=>audio.paused?audio.play():audio.pause();$('[data-back]').onclick=()=>audio.currentTime=Math.max(0,audio.currentTime-.25);$('[data-forward]').onclick=()=>audio.currentTime=Math.min(audio.duration||0,audio.currentTime+.25);$('[data-seek]').oninput=e=>{if(audio.duration)audio.currentTime=Number(e.target.value)/1000*audio.duration};
  ...
  audio.addEventListener('loadedmetadata',sync);audio.addEventListener('timeupdate',sync);audio.addEventListener('play',()=>{$('[data-play]').innerHTML='Ⅱ <span>Pause</span>'});audio.addEventListener('pause',()=>{$('[data-play]').innerHTML='▶ <span>Play</span>';sync()});canvas.onclick=e=>{if(audio.duration)audio.currentTime=e.offsetX/canvas.clientWidth*audio.duration};addEventListener('resize',drawWave);
